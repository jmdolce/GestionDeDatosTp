...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Agente
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Agente
...
-- CHEQUEAR
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Provincia
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Provincia(nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Localidad
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Localidad(nombre, provincia_id) 
...
-- CHEQUEAR
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Barrio
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Barrio (nombre, localidad_id)
...
-- CHEQUEAR
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Sucursal
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Sucursal
    ...
    FROM gd_esquema.Maestra m
    JOIN BI_LOS_GDDS.BI_Localidad l ON m.SUCURSAL_LOCALIDAD = l.nombre  
END
GO


CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Propietario
AS
BEGIN
    INSERT INTO BI_LOS_GDDS.BI_Propietario (id, nombre, apellido, dni, fecha_registro, telefono, mail, fecha_nacimiento)
    SELECT DISTINCT
...
------------- INMUEBLE ----------------
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Disposicion
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Disposicion (nombre)
...
CREATE PROCEDUREBI_ LOS_GDDS.MIGRAR_BI_EstadoInmueble
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.Estado_BI_inmueble (nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Orientacion
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Orientacion (nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_TipoInmueble
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Tipo_inmueble(nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Ambiente
AS
BEGIN
    INSERT INTO BI_LOS_GDDS.BI_Ambiente(nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Inmueble
AS
BEGIN
    INSERT INTO BI_LOS_GDDS.BI_Inmueble(id, tipo_inmueble_id, descripcion, direccion, propietario_id, barrio_id, superficie_total, disposicion_id, estado_id, orientacion_id, antiguedad, ultima_expensa)
    SELECT
        m.INMUEBLE_CODIGO,
        MAX(ti.id) AS id,
    ...
    FROM gd_esquema.Maestra m
    JOIN BI_LOS_GDDS.BI_Propietario p ON m.PROPIETARIO_DNI = p.dni
    JOIN BI_LOS_GDDS.BI_Disposicion d ON m.INMUEBLE_DISPOSICION = d.nombre
    JOIN BI_LOS_GDDS.BI_Estado_inmueble e ON m.INMUEBLE_ESTADO = e.nombre
    JOIN BI_LOS_GDDS.BI_Orientacion o ON m.INMUEBLE_ORIENTACION = o.nombre
    JOIN BI_LOS_GDDS.BI_Barrio b ON m.INMUEBLE_BARRIO = b.nombre
    JOIN BI_LOS_GDDS.BI_Tipo_inmueble ti ON m.INMUEBLE_TIPO_INMUEBLE = ti.nombre
    WHERE INMUEBLE_CODIGO IS NOT NULL
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_AmbienteInmueble
AS
BEGIN
    INSERT INTO BI_LOS_GDDS.BI_Inmueble_Ambiente(ambiente_id, inmueble_id)
    SELECT DISTINCT a.id, i.id
    FROM gd_esquema.Maestra m
    JOIN BI_LOS_GDDS.BI_Inmueble i ON m.INMUEBLE_CODIGO = i.id
    JOIN BI_LOS_GDDS.BI_Ambiente a ON a.nombre = INMUEBLE_CANT_AMBIENTES
    WHERE INMUEBLE_CODIGO IS NOT NULL;
END
GO
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Caracteristica
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Caracteristica(nombre)
	VALUES('wifi'), ('cable'), ('calefaccion'), ('gas')
END
GO


CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_CaracteristicaInmueble
AS 
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            INSERT INTO BI_LOS_GDDS.BI_Caracteristica_inmueble(caracteristica_id, inmueble_id)
            SELECT DISTINCT c.id, i.id
            FROM BI_LOS_GDDS.BI_Inmueble i
            JOIN BI_LOS_GDDS.BI_Caracteristica c ON c.nombre = 'wifi'
            WHERE i.id = @InmuebleCodigo
            AND NOT EXISTS (
                SELECT 1
                FROM BI_LOS_GDDS.BI_Caracteristica_inmueble ci
        ...
        BEGIN
            ...
            SELECT DISTINCT c.id, i.id
            FROM BI_LOS_GDDS.BI_Inmueble i
            JOIN BI_LOS_GDDS.BI_Caracteristica c ON c.nombre = 'cable'
            WHERE i.id = @InmuebleCodigo
            AND NOT EXISTS (
                SELECT 1
                FROM BI_LOS_GDDS.BI_Caracteristica_inmueble ci
        ...
        BEGIN
            ...
            SELECT DISTINCT c.id, i.id
            FROM BI_LOS_GDDS.BI_Inmueble i
            JOIN BI_LOS_GDDS.BI_Caracteristica c ON c.nombre = 'calefaccion'
            WHERE i.id = @InmuebleCodigo
            AND NOT EXISTS (
                SELECT 1
                FROM BI_LOS_GDDS.BI_Caracteristica_inmueble ci
        ...
        BEGIN
            ...
            SELECT DISTINCT c.id, i.id
            FROM BI_LOS_GDDS.BI_Inmueble i
            JOIN BI_LOS_GDDS.BI_Caracteristica c ON c.nombre = 'gas'
            WHERE i.id = @InmuebleCodigo
            AND NOT EXISTS (
                SELECT 1
                FROM BI_LOS_GDDS.BI_Caracteristica_inmueble ci
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Anuncio
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Anuncio(id, fecha_publicacion, agente_id, inmueble_id, operacion_id, precio_inmueble, moneda_id, periodo_id, estado_id, fecha_finalizacion, costo_publicacion)
    ...
    FROM gd_esquema.Maestra m
    JOIN BI_LOS_GDDS.BI_Operacion o ON m.ANUNCIO_TIPO_OPERACION = o.nombre
    JOIN BI_LOS_GDDS.BI_Agente ag ON m.AGENTE_DNI = ag.dni
    JOIN BI_LOS_GDDS.BI_Moneda mon ON m.ANUNCIO_MONEDA = mon.nombre
    JOIN BI_LOS_GDDS.BI_Tipo_periodo tp ON m.ANUNCIO_TIPO_PERIODO = tp.nombre
    JOIN BI_LOS_GDDS.BI_Estado_anuncio ea ON m.ANUNCIO_ESTADO = ea.nombre
    
    WHERE m.INMUEBLE_CODIGO IS NOT NULL
END
GO

CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_EstadoAnuncio
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Estado_anuncio
        (nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Operacion  
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Operacion
        (nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_TipoPeriodo
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Tipo_periodo
        (nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Comprador
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Comprador
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_MedioPago
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Medio_pago
        (nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_PagoVenta
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Pago_venta
    ...
    FROM gd_esquema.Maestra m
    JOIN BI_LOS_GDDS.BI_Moneda mon ON mon.nombre = m.PAGO_VENTA_MONEDA
    JOIN BI_LOS_GDDS.BI_Medio_pago mp ON mp.nombre = m.PAGO_VENTA_MEDIO_PAGO
END
GO


 CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_PagoAlquiler
 AS 
 BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Pago_alquiler
    ...
    FROM gd_esquema.Maestra m
    JOIN BI_LOS_GDDS.BI_Medio_pago mp ON mp.nombre = m.PAGO_ALQUILER_MEDIO_PAGO
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Venta
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Venta
    ...
    FROM gd_esquema.Maestra m
    JOIN BI_LOS_GDDS.BI_Comprador c ON c.nombre = m.COMPRADOR_NOMBRE AND c.apellido = m.COMPRADOR_APELLIDO AND c.dni = m.COMPRADOR_DNI
    JOIN BI_LOS_GDDS.BI_Moneda mon ON mon.nombre = m.VENTA_MONEDA
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Alquiler
AS
BEGIN
    ;WITH CTE AS (
        SELECT
        ...
        FROM gd_esquema.Maestra m
        JOIN BI_LOS_GDDS.BI_Estado_alquiler e ON m.ALQUILER_ESTADO = e.nombre
        JOIN BI_LOS_GDDS.BI_Inquilino inq ON m.INQUILINO_DNI = inq.dni
        WHERE m.ALQUILER_CODIGO IS NOT NULL
    )
    
    INSERT INTO BI_LOS_GDDS.BI_Alquiler(id, fecha_inicio, estado_id, fecha_fin, cantidad_periodos, comision, gastos_averiguaciones, deposito, anuncio_id, importe, inquilino_id)
    SELECT
        ALQUILER_CODIGO,
...
GO
 ...
 CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_DetalleAlquiler
 AS 
 BEGIN
 	INSERT INTO BI_LOS_GDDS.BI_Detalle_alquiler(periodo_inicio, periodo_fin, precio, alquiler_id)
 	SELECT DISTINCT m.DETALLE_ALQ_NRO_PERIODO_INI, m.DETALLE_ALQ_NRO_PERIODO_FIN, m.DETALLE_ALQ_PRECIO, m.ALQUILER_CODIGO
    FROM gd_esquema.Maestra m
 END
 GO

CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_Inquilino
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Inquilino
        (nombre, apellido, dni, fecha_registro, telefono, mail, fecha_nacimiento)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_EstadoAlquiler
AS 
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_Estado_alquiler
        (nombre)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_RANGO_ETARIO
AS
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_RANGO_ETARIO(RANGO_ETARIO_DESCRIPCION)
		VALUES('<25')
	INSERT INTO BI_LOS_GDDS.BI_RANGO_ETARIO(RANGO_ETARIO_DESCRIPCION)
		VALUES('25-35')
	INSERT INTO BI_LOS_GDDS.BI_RANGO_ETARIO(RANGO_ETARIO_DESCRIPCION)
...
CREATE PROCEDURE BI_LOS_GDDS.MIGRAR_BI_RANGO_M2
AS
BEGIN
	INSERT INTO BI_LOS_GDDS.BI_RANGO_M2(RANGO_M2_DESCRIPCION)
		VALUES('<25')
	INSERT INTO BI_LOS_GDDS.BI_RANGO_M2(RANGO_M2_DESCRIPCION)
		VALUES('25-35')
	INSERT INTO BI_LOS_GDDS.BI_RANGO_M2(RANGO_M2_DESCRIPCION)
